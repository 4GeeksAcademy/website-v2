...
walk(`${__dirname}/../data/location`, async (err, files) => {
    ...
    for (let i = 0; i < _files.length; i++) {
        ...
        if (slugs.length === _files.length)
            checkForLanguages(slugs, "location");
        ...
        try {
            ...
            content_fields.forEach((obj) => {
                ...
                if (!meta_keys.includes(obj["key"]))
                    fail(
                        `Missing prop ${obj["key"]} from location on ${_path}`
                    );
                else {
                    if (obj["type"] === "string") {
                        if (
                            ...
                            fail(
                                `\n\nInvalid mandatory prop ${
                                    ...
                                    location[obj["key"]].yellow
                                } ${"match with".red} ${_slug.yellow}\n\n`
                            );
                    }
                }
            });
            ...
            locations_fields.forEach((field) => {
                if (
                    !meta_keys.includes(field["key"]) &&
                    field["mandatory"] === true
                )
                    warn(
                        `Missing prop ${field["key"]} from location on ${_path}`
                    );
