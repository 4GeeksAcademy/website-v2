...
const { GATSBY_BREATHECODE_HOST, GATSBY_BLOG_ACADEMY_TOKEN, GATSBY_BLOG_ACADEMY_ID } = process.env;
...
const getPosts = async () => {
    ...
    const result = await axios.get(GATSBY_BREATHECODE_HOST+'/registry/academy/asset?category=blog-es,blog-us', options)
...
const getContent = async (slug) => {
    ...
    try{
        ...
        const _resp = await axios.get(GATSBY_BREATHECODE_HOST+`/registry/asset/${slug}.raw`, options);
        if(_resp.status != 200){
