  ...
  <StaticQuery
    query={graphql`
      query DownloadableQuery {
        allDownloadableYaml(sort: {fields: meta_info___current_download, order: ASC}) {
            edges {
              node {
                meta_info {
                  slug
      ...
      >
        <Div>
          ...
          <Table>
            ...
            <tbody>
              {downs && downs.map(({node}) =>
                <tr>
                  <td><Anchor to={`/${node.fields.lang}/downloadable/${node.meta_info.slug}`}>{node.meta_info.slug}</Anchor></td>
