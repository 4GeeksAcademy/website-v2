...
const UpcomingProgram = ({upcomingPath, position, showOnScrollPosition, button, lang}) => {
    ...
    return (<ShadowedRow width="100%" position={position} hide={!show}>
        <Centered>
            <Left>
                <CalendarIcon month={dayjs(date).format('MMM')} day={dayjs(date).add(1, 'day').format('D')} height="60px" />
            </Left>
            <Center>
                <P1 fontSize="30px">{button.next_cohort}{" "}{dayjs(date).add(1, 'day').format('MMM D, YYYY')}</P1>
