...
const FaqCard = ({ faqs, topicSlug, locationSlug, minPriority }) => {
  ...
  const filterByLocation = (question) => {
    ...
    if (Array.isArray(question.locations) && question.locations.includes(locationSlug)) return true;
    return false;
  };
  ...
  const filteredTopics = faqs.filter((topic) => topicSlug ? topic.slug === topicSlug : true);
  filteredTopics.forEach(element => {
    element.questions = element.questions.filter(filterByLocation).filter(filterByPriority);
  });

  return (
    <Div display="block">
      {filteredTopics.map((item, i) => item.questions.length > 0 && (
