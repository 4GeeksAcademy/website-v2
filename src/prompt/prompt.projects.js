...
async function generate(){
  ...
  const doc = loadYML(`${__dirname}/../data/components/alumni_projects/alumni_projects.us.yaml`);
  let projects = doc.yaml.projects.map((old) => {
    let _new = [
      ...
      `Buit by: ${old.alumni.map(a => `${a.first_name} ${a.last_name}`).join(' and ')} during the ${old.course} bootcamp`,
    ]
    // if(old.linkedin_url) _new.push(`LinkedIn profile URL: ${old.linkedin_url}`);
    return _new.join("\n");
  })
  
  fs.writeFileSync(`./prompts/projects.prompt`, [
    `The following are capstone projects built by our students during our bootcamps:`,projects.join("\n\n---\n\n"),
    ``
  ].join("\n\n"), 'utf8');
