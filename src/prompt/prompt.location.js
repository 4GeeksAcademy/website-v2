...
async function generate(){
  ...
  for(let locationSlug of allSlugs){
    ...
    campuses[locationSlug].website = `https://4geeksacademy.com/us/coding-campus/${old.meta_info.slug}`;;
    campuses[locationSlug].social_media_urls = old.socials.map(s => ({ network: s.name, url: s.link }));
  
    const raw = toYML(campuses[locationSlug])

    console.log(`Generating summary for campus ${locationSlug}.`)
    allLocationsPrompts += '\n\nNext campus: '+await complete({
      max_tokens,
      ...
      user: `The followings are the details about 4Geeks Academy ${old.city}, ${old.country},
  ...
  fs.writeFileSync(`./prompts/locations.prompt`, "4Geeks Academy is available in the following locations:"+allLocationsPrompts, 'utf8');
